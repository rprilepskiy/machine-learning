...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing == True:
            ...
            # self.epsilon -= 0.01
            # self.trials += 1
            # self.epsilon = 1 - math.pow(self.alpha, 1/math.sqrt(self.trials)) 
            self.epsilon = math.exp(-3*self.const*self.trials) 
            # self.epsilon = 1 / (1 + self.alpha*self.trials)
            self.trials += 1
    ...
    def choose_action(self, state):
        ...
        if self.learning == True:
            ...
            print(self.epsilon, rand_number)
            if self.epsilon > rand_number:
                action = random.choice(self.valid_actions)
            else:   
                maxQ = self.get_maxQ(state)
                good_actions = [key for key, val in self.Q[state].items() if val == maxQ]
